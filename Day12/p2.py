...
def calc_sides(region):
    ...
    ans = 0
    for r, c in tocheck:
        status = [(cr, cc) in region for cr, cc in [(r + 0.5, c + 0.5), (r - 0.5, c + 0.5), (r - 0.5, c - 0.5 ), (r + 0.5, c - 0.5)]]
        if sum(status) in [1,3]:
            ans += 1
        elif status in [[True, False, True, False], [False, True, False, True]]:
            ans += 2
    return ans


print(sum(len(region) * calc_sides(region) for region in regions))