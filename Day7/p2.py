...
def possible(target, array):
    ...
    lastString = str(array[-1])
    if targetString.endswith(lastString) and len(targetString) > len(lastString) and possible(int(targetString[:-len(lastString)]), array[:-1]): return True
    return False
